# AI-ClaudeCat v4.0 - 项目文档

**版本**: v4.0.0  
**最后更新**: 2026-02-06  
**状态**: 🚀 重构中（采用日志监控方案）

---

## 📋 目录

1. [项目概述](#项目概述)
2. [核心特性](#核心特性)
3. [技术架构](#技术架构)
4. [快速开始](#快速开始)
5. [配置说明](#配置说明)
6. [开发指南](#开发指南)
7. [API 文档](#api-文档)
8. [版本历史](#版本历史)

---

## 项目概述

AI-ClaudeCat 是一款专为 **Claude Code** 设计的智能状态监控工具，通过监控官方 JSONL 日志文件，实时追踪 AI 的运行状态、工具调用和 Token 使用情况。

### 核心理念

- **可靠性优先**: 采用 Claude Code 官方日志数据源（已验证，格式稳定）
- **工具级精度**: 不仅知道 AI 在"工作"，还知道在"读文件"还是"写代码"
- **隐私保护**: 内置白名单过滤机制，只输出元数据
- **插件化架构**: 易扩展，支持更多 AI 编程工具

### 应用场景

1. **桌面宠物 GUI** - 实时显示 AI 状态的可爱宠物
2. **浏览器插件** - 在浏览器中展示 AI 工作状态
3. **移动端 App** - 远程监控 AI 编程进度
4. **数据统计** - Token 使用量、工具调用频率分析
5. **开发调试** - 了解 AI 的工作流程和决策

---

## 核心特性

### ✅ 已实现（v4.0）

#### 1. **日志监控**（核心）
- 📁 监控 `~/.claude/projects/**/*.jsonl`
- 🔄 增量读取（记录文件位置，低 CPU 占用）
- 📝 JSONL 解析（官方格式，稳定可靠）
- 🎯 实时性高（文件变化立即触发）

#### 2. **工具级状态检测**
- **Thinking** - AI 思考中（`thinking` 块）
- **Responding** - AI 回复中（`text` 块）
- **Reading** - 读取文件（`Read` 工具）
- **Writing** - 写入文件（`Write` 工具）
- **Editing** - 编辑文件（`Edit` 工具）
- **Executing** - 执行命令（`Bash` 工具）
- **Searching** - 搜索代码（`Grep`、`Glob` 工具）
- **Fetching** - 网络请求（`WebFetch`、`WebSearch` 工具）
- **Spawning** - 派生子 Agent（`Task` 工具）
- **Waiting** - 等待用户输入（`AskUserQuestion` 工具）

#### 3. **Token 统计**
- 📊 输入 Token（input_tokens）
- 📊 输出 Token（output_tokens）
- 📊 缓存读取（cache_read_input_tokens）
- 📊 缓存写入（cache_creation_input_tokens）
- 📊 缓存命中率计算
- 📊 平均每分钟使用量

#### 4. **隐私保护**
- 🔒 白名单过滤（只输出允许的字段）
- 🔒 文件路径 → 只保留文件名
- 🔒 命令内容 → 不输出
- 🔒 文件内容 → 不输出
- 🔒 只保留元数据（工具名称、状态、Token）

#### 5. **多输出协议**
- 🌐 **WebSocket** (ws://127.0.0.1:8765) - 实时推送
- 🌐 **HTTP REST API** (http://127.0.0.1:8080) - 查询接口
- 📝 **Stdout** - 终端输出（调试）
- 💾 **SQLite** - 历史记录存储

#### 6. **事件历史**
- 💾 SQLite 数据库存储
- 🔍 时间范围查询
- 📊 统计分析接口
- 📈 Token 使用趋势

---

## 技术架构

### v4.0 架构图

```
┌─────────────────────────────────────────────────────────────┐
│                     AI-ClaudeCat v4.0                        │
│                 (基于 PixelHQ-bridge 方案)                   │
└─────────────────────────────────────────────────────────────┘

数据源层
┌──────────────────────────────────────────┐
│  ~/.claude/projects/**/*.jsonl           │
│  - session-abc123.jsonl                  │
│  - session-abc123/subagents/xyz.jsonl    │
└──────────────────────────────────────────┘
           │
           ▼
监控层
┌──────────────────────────────────────────┐
│  ClaudeLogPlugin (核心插件) ⭐            │
│  - Watchdog 文件监控                      │
│  - 增量读取 (filePositions)              │
│  - JSONL 解析                            │
│  - 状态推断 (thinking/tool_use)          │
│  - Token 统计                            │
│  - Agent 追踪                            │
└──────────────────────────────────────────┘
           │ emit(StateEvent)
           ▼
中间件层
┌──────────────────────────────────────────┐
│  Middleware                              │
│  ├── EventBus (事件分发)                 │
│  ├── StateFusion (状态融合)              │
│  ├── PrivacyFilter (隐私过滤) ⭐          │
│  └── TokenStats (Token 统计) ⭐           │
└──────────────────────────────────────────┘
           │
           ▼
输出层
┌──────────────────────────────────────────┐
│  OutputAdapters                          │
│  ├── WebSocketAdapter (ws://8765)        │
│  ├── HTTPAdapter (http://8080)           │
│  ├── StdoutAdapter (终端输出)            │
│  └── HistoryAdapter (SQLite) ⭐           │
└──────────────────────────────────────────┘
           │
           ▼
消费端
┌──────────────────────────────────────────┐
│  - 桌面宠物 GUI (Electron/Qt)            │
│  - 浏览器插件 (Chrome Extension)         │
│  - 移动端 App (React Native)             │
│  - CLI 工具                              │
└──────────────────────────────────────────┘
```

### 数据流

```
文件变化 (*.jsonl)
    │
    ▼
Watchdog 监控
    │ on_modified
    ▼
ClaudeLogPlugin
    │
    ├─► 增量读取新行
    ├─► 解析 JSONL
    ├─► 推断状态
    └─► emit(StateEvent)
           │
           ▼
Middleware
           │
           ├─► PrivacyFilter  # 隐私过滤
           ├─► TokenStats      # Token 统计
           └─► StateFusion     # 状态融合
                   │
                   ▼
           for adapter in adapters:
               await adapter.send(event)
                   │
                   ├─► WebSocket → 前端
                   ├─► HTTP → REST API
                   ├─► Stdout → 终端
                   └─► History → SQLite
```

### 目录结构

```
AI-ClaudeCat/
├── main.py                    # 主程序入口
├── config.json                # 配置文件
├── requirements.txt           # 依赖清单
├── CLAUDE.md                  # 本文件
├── AGENTS.md                  # 项目知识库
├── README.md                  # 项目总览
├── QUICKSTART.md              # 快速开始
├── CONFIG.md                  # 配置说明
│
├── src/                       # 源代码
│   ├── plugins/               # 插件实现
│   │   ├── base.py           # BasePlugin、StateEvent、Status
│   │   └── claude_log.py     # ClaudeLogPlugin（核心）⭐
│   │
│   ├── middleware/            # 中间件
│   │   ├── core.py           # Middleware 主逻辑
│   │   ├── event_bus.py      # EventBus 事件分发
│   │   ├── fusion.py         # StateFusion 状态融合
│   │   ├── privacy.py        # PrivacyFilter 隐私过滤 ⭐
│   │   └── token_stats.py    # TokenStats Token 统计 ⭐
│   │
│   ├── adapters/              # 输出适配器
│   │   ├── base.py           # OutputAdapter 基类
│   │   ├── websocket_adapter.py  # WebSocket 实时推送
│   │   ├── http_adapter.py   # HTTP REST API
│   │   ├── stdout_adapter.py # 标准输出
│   │   └── history_adapter.py # SQLite 存储 ⭐
│   │
│   └── utils/                 # 工具模块
│       ├── window_detector.py # 窗口检测（保留，暂不实装）
│       └── README.md          # 工具说明
│
├── docs/                      # 文档
│   └── research_notes.md      # 技术研究笔记
│
├── data/                      # 数据目录
│   └── history.db             # 事件历史数据库
│
└── backup-v3.1/               # v3.1 备份
    ├── src/                   # 旧插件代码
    ├── docs/                  # 旧文档
    └── Desktop-Pixel-Pet/     # 其他项目备份
```

---

## 快速开始

### 1. 安装依赖

```bash
pip install -r requirements.txt
```

**依赖列表**:
- `watchdog` - 文件监控
- `websockets` - WebSocket 服务器
- `flask` - HTTP 服务器
- `flask-cors` - CORS 支持
- `psutil` - 进程监控（可选，用于进程存活检测）

### 2. 配置

编辑 `config.json`：

```json
{
  "version": "4.0.0",
  "claude": {
    "projects_dir": "auto"  // 自动检测，或指定路径
  },
  "middleware": {
    "privacy_filter": {
      "enabled": true  // 启用隐私保护
    }
  }
}
```

### 3. 运行

```bash
python main.py
```

**输出**:
```
=== AI-ClaudeCat v4.0 ===
Status monitoring for Claude Code

✓ Claude Code detected at C:\Users\YourName\.claude\projects
✓ Middleware initialized
✓ WebSocket server on port 8765
✓ HTTP server on port 8080
✓ Privacy filter enabled
✓ History storage enabled

[ClaudeLogPlugin] Started, watching: C:\Users\YourName\.claude\projects
```

### 4. 测试

**WebSocket 客户端** (浏览器控制台):
```javascript
const ws = new WebSocket('ws://127.0.0.1:8765');
ws.onmessage = (event) => {
    console.log('AI Status:', JSON.parse(event.data));
};
```

**HTTP 查询**:
```bash
# 当前状态
curl http://127.0.0.1:8080/api/status

# 历史事件
curl http://127.0.0.1:8080/api/history?limit=10

# Token 统计
curl http://127.0.0.1:8080/api/tokens
```

---

## 配置说明

详细配置请参考 [`CONFIG.md`](./CONFIG.md)

### 核心配置项

#### Claude 配置
```json
{
  "claude": {
    "projects_dir": "auto",           // "auto" 或指定路径
    "watch_debounce_ms": 100,         // 文件变化防抖（毫秒）
    "session_ttl_minutes": 10         // 会话存活时间（分钟）
  }
}
```

#### 隐私过滤配置
```json
{
  "middleware": {
    "privacy_filter": {
      "enabled": true,
      "whitelist": [
        "method", "event", "tool", "context",
        "session_id", "status", "confidence",
        "tokens", "agent_type", "pattern"
      ]
    }
  }
}
```

#### 输出适配器配置
```json
{
  "adapters": {
    "websocket": {
      "enabled": true,
      "port": 8765,
      "host": "127.0.0.1"
    },
    "http": {
      "enabled": true,
      "port": 8080,
      "host": "127.0.0.1",
      "cors": true
    },
    "history": {
      "enabled": true,
      "db_path": "data/history.db",
      "max_events": 10000
    }
  }
}
```

---

## 开发指南

### 添加新插件

```python
# src/plugins/my_plugin.py
from src.plugins.base import BasePlugin, StateEvent, Status

class MyPlugin(BasePlugin):
    def __init__(self, name: str = "my_plugin"):
        super().__init__(name)
    
    def check_available(self) -> bool:
        # 检查插件是否可用
        return True
    
    async def detect(self) -> Optional[StateEvent]:
        # 检测状态
        return StateEvent(
            status=Status.WORKING,
            confidence=0.90,
            source_plugin=self.name,
            details={"info": "..."}
        )
```

### 添加新输出适配器

```python
# src/adapters/my_adapter.py
from src.adapters.base import OutputAdapter
from src.plugins.base import StateEvent

class MyAdapter(OutputAdapter):
    def __init__(self):
        super().__init__("my_adapter")
    
    async def send(self, event: StateEvent):
        # 发送事件到目标
        print(f"Sending to my service: {event.to_dict()}")
```

### 事件格式

**StateEvent**:
```python
{
    "timestamp": "2026-02-06T12:34:56.789Z",
    "status": "working",                    # Status 枚举
    "confidence": 0.95,                      # 0.0 - 1.0
    "source_plugin": "claude_log",           # 插件名称
    "source_type": "custom",                 # PluginType 枚举
    "priority": 10,                          # 优先级
    "details": {                             # 额外信息
        "method": "log",
        "event": "tool_use",
        "tool": "Read",
        "context": "main.py",
        "session_id": "abc123"
    }
}
```

---

## API 文档

### WebSocket API

**连接**: `ws://127.0.0.1:8765`

**消息格式**:
```json
{
    "type": "state_change",
    "data": {
        "timestamp": "2026-02-06T12:34:56.789Z",
        "status": "working",
        "confidence": 0.95,
        "details": {
            "tool": "Read",
            "context": "main.py"
        }
    }
}
```

### HTTP REST API

#### GET /api/status
获取当前状态

**响应**:
```json
{
    "status": "working",
    "confidence": 0.95,
    "timestamp": "2026-02-06T12:34:56.789Z",
    "details": {
        "tool": "Read",
        "context": "main.py"
    }
}
```

#### GET /api/history
查询历史事件

**参数**:
- `start_time`: ISO 8601 时间戳（可选）
- `end_time`: ISO 8601 时间戳（可选）
- `limit`: 返回数量（默认 100）

**响应**:
```json
{
    "events": [
        {
            "id": 1,
            "timestamp": "2026-02-06T12:34:56.789Z",
            "status": "working",
            "confidence": 0.95,
            "details": {...}
        }
    ],
    "total": 1
}
```

#### GET /api/tokens
获取 Token 统计

**响应**:
```json
{
    "total": {
        "input": 50000,
        "output": 20000,
        "cache_read": 10000,
        "cache_write": 5000
    },
    "runtime_seconds": 3600,
    "average_per_minute": {
        "input": 833.33,
        "output": 333.33
    },
    "cache_hit_rate": 0.167
}
```

---

## 版本历史

### v4.0.0 (2026-02-06) - 🚀 重大重构

**核心改动**:
- ✅ 采用日志监控方案（借鉴 PixelHQ-bridge）
- ✅ 弃用系统 API 检测（窗口标题、CPU 阈值）
- ✅ 增加工具级状态检测
- ✅ 增加 Token 统计功能
- ✅ 增加隐私保护机制
- ✅ 增加事件历史存储

**新增功能**:
- 📁 ClaudeLogPlugin（日志监控插件）
- 🔒 PrivacyFilter（隐私过滤器）
- 📊 TokenStats（Token 统计器）
- 💾 HistoryAdapter（SQLite 历史存储）

**移除功能**:
- ❌ WindowPlugin（窗口标题检测）- 保留为工具模块
- ❌ ProcessPlugin（CPU 阈值检测）
- ❌ ClaudeCodePlugin（旧融合检测）

### v3.1.0 (2026-02-05) - 已归档

**特性**:
- 窗口标题检测
- 进程 CPU 监控
- 文件活动猜测
- 多方式融合

**问题**:
- 窗口标题不包含状态信息
- CPU 阈值不准确
- 文件监控未实现
- 误判严重

**备份位置**: `backup-v3.1/`

---

## 致谢

本项目的日志监控方案受到 [PixelHQ-bridge](https://github.com/example/pixelhq-bridge) 的启发，感谢其开源贡献。

---

## 许可证

MIT License

---

**最后更新**: 2026-02-06  
**维护者**: AI-ClaudeCat Team  
**问题反馈**: [GitHub Issues](https://github.com/example/ai-claudecat/issues)
