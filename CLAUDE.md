# Claude 桌面宠物项目

**状态**：v3.1 核心实现中
**最后更新**：2026-02-04

---

## 中文宪法

1. **语言规则**：始终使用中文进行对话、输出和文档编写
2. **代码注释**：代码文件使用中文注释（类说明、函数说明）
3. **文档规范**：所有文档（.md 文件）必须使用中文
4. **命名规范**：项目特定词汇使用中文（如"进程监控"、"状态融合"）
5. **例外情况**：代码本身保持英文（变量名、函数名、类名），标准库/API 保持英文术语

> **违反示例**：创建英文文档、使用英文注释  
> **正确示例**：创建中文文档、使用中文注释

---

## 项目概述

开发一款桌面宠物应用，实时监控 AI 编程工具（Claude Code / OpenCode / Cursor 等）的运行状态，并通过中间件架构输出状态，由前端显示对应的动画效果。

**核心特点**：
- **插件化监控架构** - 支持多种软件的检测插件，可自由扩展
- **中间件状态聚合** - 统一的事件总线和状态融合
- **多模式输出** - WebSocket / HTTP / Redis 等多种输出方式
- **前端解耦** - 前端通过标准协议接收状态，独立演化

---

## 架构设计

```
┌────────────────────────┐          ┌────────────────────────┐
│   后端 Middleware     │          │     前端 Frontend      │
├────────────────────────┤          ├────────────────────────┤
│  Plugin Layer         │  JSON    │  State Manager        │
│  ├─ Claude Plugin    │─────────►│  Animation Engine    │
│  ├─ OpenCode Plugin  │          │  GIF Display         │
│  ├─ Cursor Plugin    │          │  CSS Effects         │
│  └─ Custom Plugin    │          │                      │
│                        │          │                      │
│  Middleware Core      │          │                      │
│  ├─ State Pool       │          │                      │
│  ├─ Event Bus        │          │                      │
│  └─ Output Adapters  │          │                      │
│     ├─ WebSocket     │          │                      │
│     ├─ HTTP REST     │          │                      │
│     └─ Redis Pub/Sub │          │                      │
└────────────────────────┘          └────────────────────────┘
```

---

## 核心组件

| 组件 | 职责 | 状态 |
|------|------|------|
| **Plugin Base** | 插件基类接口 | 设计完成 |
| **ProcessPlugin** | 进程监控（CPU/内存） | 可用 |
| **WindowPlugin** | 窗口监控（标题/文本） | 可用 |
| **Middleware Core** | 状态聚合、事件总线 | 设计完成 |
| **Output Adapters** | WebSocket/HTTP/Redis 输出 | 设计完成 |
| **Frontend Core** | 前端状态管理 | 待开发 |
| **Animation Engine** | 动画播放引擎 | 待开发 |

---

## 插件系统

### 插件类型

| 类型 | 检测方式 | 适用软件 |
|------|----------|----------|
| **ProcessPlugin** | 进程 CPU/内存 | Claude Code、OpenCode、Cursor |
| **WindowPlugin** | 窗口标题/文本 | 终端、IDE、聊天软件 |
| **FilePlugin** | 文件变更 | Git、文件同步 |
| **HttpPlugin** | HTTP API | Web 服务 |
| **CustomPlugin** | 自定义逻辑 | 特殊需求 |

### 插件接口

```python
class BasePlugin(ABC):
    @abstractmethod
    def check_available(self) -> bool:
        """检查目标是否可用"""
    
    @abstractmethod
    def detect(self) -> StateEvent:
        """检测状态"""
    
    def on_event(self, event: StateEvent):
        """接收其他插件事件（可选）"""
```

### 独立运行

插件支持独立运行，方便开发和测试。

```bash
# 运行进程监控插件（默认配置）
python -m src.plugins.process

# 指定监控进程名称
python -m src.plugins.process --process-names claude,anthropic

# 指定检测间隔（秒）
python -m src.plugins.process --interval 1.0

# 显示帮助
python -m src.plugins.process --help
```

**运行效果**：
```
[2024-01-01 12:00:00] IDLE (confidence: 90%) - cpu: 0.3%
[2024-01-01 12:00:02] THINKING (confidence: 80%) - cpu: 8.5%
[2024-01-01 12:00:04] WORKING (confidence: 85%) - cpu: 35.2%
...
```

---

## 输出模式

| 模式 | 协议 | 适用场景 | 延迟 |
|------|------|----------|------|
| **WebSocket** | ws:// | 实时前端推送 | < 10ms |
| **HTTP REST** | http:// | 轮询获取状态 | 轮询间隔 |
| **Redis Pub/Sub** | redis:// | 多实例同步 | < 1ms |
| **MQTT** | mqtt:// | IoT 设备 | < 100ms |
| **WebHook** | http:// | 外部服务集成 | 网络延迟 |

---

## 技术栈

| 类别 | 选择 |
|------|------|
| **语言** | Python 3.9+ |
| **进程监控** | psutil |
| **文件监控** | watchdog |
| **WebSocket** | websockets |
| **HTTP** | Flask |
| **Redis** | redis-py |
| **前端** | WebView / Electron / tkinter |

---

## 目录结构

```
AI-ClaudeCat/
├── CLAUDE.md                              # 本文档 ⭐
├── docs/
│   ├── 完整架构设计.md                    # v3.0 总体架构
│   ├── 插件化架构详细设计.md              # v3.1 插件和输出详细设计 ⭐
│   └── research_notes.md                  # 技术研究笔记
├── docs-archive/                          # 历史文档
│   └── old-prototype-v1.0/                # v1.0 原型 (已废弃)
│       ├── VERSION.json                   # 版本元信息
│       ├── README.md                      # 归档说明
│       ├── EXPERIENCE_SUMMARY.md          # 经验总结 ⭐
│       └── src/monitor/                  # 旧版原型代码
├── src/                                  # 源代码
│   ├── plugins/                          # 插件 (v3.1)
│   │   ├── __init__.py
│   │   ├── __main__.py                   # 插件独立运行入口
│   │   ├── base.py                      # 插件基类 ⭐
│   │   ├── process.py                   # 进程监控插件
│   │   └── window.py                    # 窗口监控插件
│   ├── middleware/                       # 中间件 (待实现)
│   │   ├── core.py
│   │   ├── event_bus.py
│   │   └── fusion.py
│   └── adapters/                        # 输出适配器 (待实现)
│       ├── websocket.py
│       ├── http.py
│       └── redis.py
└── frontend/                            # 前端 (待开发)
```

---

## 开发状态

### 已完成 ✅

- [x] 架构设计（v3.1 插件化架构）
- [x] 插件接口设计文档
- [x] 输出模式设计文档
- [x] 技术研究笔记
- [x] 插件基类 (src/plugins/base.py)
- [x] 进程监控插件 (src/plugins/process.py)

### 进行中 🔄

- [ ] 实现中间件核心
- [ ] 实现 WebSocket 输出适配器
- [ ] 实现 Claude Code 插件
- [ ] 实现 OpenCode 插件

### 待完成 ⏳

- [ ] HTTP REST 输出适配器
- [ ] Redis Pub/Sub 输出适配器
- [ ] 前端 GUI
- [ ] 动画引擎
- [ ] 配置系统

### 已废弃 ❌

- [x] v1.0 原型 (src/monitor/, scripts/) → docs-archive/old-prototype-v1.0/

---

## 状态检测能力

### 当前支持的状态

| 状态 | 说明 | 检测方式 |
|------|------|----------|
| **unknown** | 未知 | 默认 |
| **idle** | 待机/空闲 | CPU < 阈值 |
| **running** | 运行中 | CPU 低负载 |
| **thinking** | 思考中 | CPU 中负载 + 关键词 |
| **working** | 工作/忙碌 | CPU 高负载 |
| **executing** | 执行命令 | 进程活动 |
| **error** | 错误 | 窗口标题/异常 |
| **stopped** | 已停止 | 进程不存在 |

### 插件检测方式

| 方式 | 目标 | 状态 |
|------|------|------|
| **进程监控** | CPU/内存占用 | ✅ 验证 |
| **文件监控** | 目录变更 | ✅ 验证 |
| **窗口监控** | 窗口标题 | ✅ 验证 |

---

## 文档索引

| 文档 | 说明 | 优先级 |
|------|------|--------|
| **docs/插件化架构详细设计.md** | 插件设计、输出模式、开发指南 | ⭐⭐⭐ |
| **docs/完整架构设计.md** | 总体架构、组件设计 | ⭐⭐ |
| **docs/research_notes.md** | 技术研究、代码示例 | ⭐⭐ |
| **src/** | 源代码 | ⭐⭐⭐ |

---

## 注意事项

- 项目处于**核心实现阶段**，插件基类已完成
- 插件系统支持自由扩展，可适配多种软件
- 前端与后端通过标准协议解耦
- v1.0 原型已迁移至 `docs-archive/old-prototype-v1.0/`
- **迁移经验**: 详见 `docs-archive/old-prototype-v1.0/EXPERIENCE_SUMMARY.md`

---

## 插件开发

### 开发新插件

1. 确定插件类型（Process/Window/File/Http/Custom）
2. 继承对应基类
3. 实现必需方法
4. 配置元信息
5. 注册插件

### 输出集成

- 前端通过 WebSocket 连接获取实时状态
- 或通过 HTTP API 轮询获取
- 多实例通过 Redis Pub/Sub 同步

---

**文档版本**: v3.1
**最后更新**: 2026-02-04
